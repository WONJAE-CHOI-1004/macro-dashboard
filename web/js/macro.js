...
const PALETTE = ["#2456d6", "#e0403c", "#12945f", "#e8871a", "#8b4bd6", "#0f9bb5", "#d6338f", "#6b7280", "#a16207", "#15803d", "#be123c", "#4338ca", "#0369a1", "#b45309", "#7c2d92", "#334155", "#dc6803", "#0d9488", "#9333ea", "#65a30d", "#c2410c", "#1d4ed8", "#a21caf", "#475569"];
...
async function loadCountry(country) {
  ...
  if (!res.ok) {
    ...
    document.getElementById("loading").textContent = "데이터 로딩 실패: " + (err.error || res.status) + " — 새로고침(F5) 해보세요.";
...
function buildSidebar(payload) {
  ...
  root.innerHTML = "";
  const ordered = payload.series.map((s, i) => ({
    ...s,
    _idx: i
  }));
  for (const sec of [1, 2, 3, 4, 5]) {
    ...
    if (sec === 3 && payload.phillips) {
      root.appendChild(makeRow("phillips", "필립스 곡선 (산점도 보기)", "실업률(가로축)과 인플레이션(세로축)의 관계를 점으로 표시합니다. 색이 진할수록 최근입니다.", -1, sel));
    }
  }
}
...
function render() {
  ...
  const opts = {
      ...
      tooltip: {
        callbacks: {
          label(ctx) {
            ...
            const shown = ctx.dataset.normed ? `${r.toLocaleString()} ${u} (지수 ${ctx.parsed.y.toFixed(1)})` : `${r.toLocaleString()}${u === "%" ? "%" : u ? " " + u : ""}`;
    ...
    scales: {
      ...
      y: {
        ...
        grid: {
          color: c => c.tick.value === 0 ? "#94a3b8" : "#eef1f6"
        }
      },
      y2: {
        position: "right",
        ...
        title: {
          display: true,
          text: hasNorm && hasRaw ? "지수(시작=100) · 원값 혼합" : hasRaw ? "원값" : "지수 (기간 시작=100)"
        },
        grid: {
  ...
  if (sel.has("phillips") && payload.phillips) {
    ...
    document.getElementById("phillipsTitle").textContent = `필립스 곡선 산점도 — 가로: ${ph.x}, 세로: ${ph.y} (진한 점 = 최근)`;
...
function renderReport(d) {
  ...
  if (!d || d.none) {
    box.className = "report-md empty";
    box.textContent = d && d.error ? "생성 실패: " + d.error + " — 다시 시도해주세요." : "아직 생성된 리포트가 없습니다. '리포트 생성' 버튼을 눌러주세요.";
    meta.textContent = "";
    return;
  }
...
function renderFomc(d) {
  ...
  document.getElementById("fomcDecision").textContent = `⚖️ 표결 결과: ${d.decision} (위원별 1개월 결정 — ` + d.members.map(m => `${m.title}: ${m.vote.decision_1m}`).join(", ") + `) · 현재 금리 ${d.current_rate}%`;
      ...
      scales: {
        x: {
          ...
          ticks: {
            ...
            callback: v => {
              var _horizons$v;
              return (_horizons$v = horizons[v]) != null ? _horizons$v : "";
            }
          },
          grid: {
            display: false
  ...
  d.members.forEach(m => addBubble(m.stance, `${m.title} (1개월: ${m.vote.decision_1m} ${m.vote.rate_1m}% / 6개월: ${m.vote.rate_6m}% / 1년: ${m.vote.rate_1y}%)`, m.final));
...
function renderKpis() {
  ...
  specs.forEach(spec => {
    ...
    metaEl.textContent = `${last[0].slice(0, 7)} 기준` + (delta == null ? "" : ` · 전년동월대비 ${delta >= 0 ? "+" : ""}${fmt(delta)}%p`);
  });
}
...
function renderCompare() {
  ...
  let html = '<table class="compare"><tr><th>지표</th><th>현재 실제</th>' + '<th>AI 시뮬레이션<span class="src">1개월 / 6개월 / 1년</span></th>' + (hasSep ? `<th>연준 SEP<span class="src">${year}년말 / ${year + 1}년말</span></th>` : "") + `<th>IMF 전망<span class="src">${year}년 / ${year + 1}년</span></th></tr>`;
  for (const r of rows) {
  ...
  html += "</table>";
  html += `<div class="compare-note">· AI 시뮬레이션은 매파·중립·비둘기파 3인 표결의 <b>중간값</b>` + (fomc ? ` (이번 결정: ${fomc.decision})` : " (아직 실행 전이면 — 표시)") + `이고 금리에만 적용됩니다 · SEP는 FOMC 위원 전망 중간값(연말 기준)` + ` · IMF는 세계경제전망(WEO) 연평균 기준이라 서로 기준 시점이 조금 다릅니다</div>`;
  box.className = "";
  box.innerHTML = html;
}
...
async function loadAIPanels() {
  ...
  try {
    renderReport(await fetchAI("report"));
  } catch (e) {/* 무시 */}
  try {
    renderFomc(await fetchAI("fomc"));
  } catch (e) {/* 무시 */}
}
const COUNTRY_NAME = {
  us: "🇺🇸 미국",
