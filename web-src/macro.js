...
// ---------- 발표 일정 캘린더 ----------
const CAL_KIND = {
  meeting: { label: "통화정책 회의", cls: "k-meeting" },
  cpi: { label: "CPI", cls: "k-cpi" },
  pce: { label: "PCE·개인소득", cls: "k-pce" },
  jobs: { label: "고용", cls: "k-jobs" },
  gdp: { label: "GDP", cls: "k-gdp" },
};
...
function renderCalendar(reorient = false) {
  ...
  if (reorient) {
    ...
    const has = cal.events.some(
      (e) => e.date.slice(0, 7) === mm || (e.end || e.date).slice(0, 7) === mm);
    if (!has) {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const cls = [
      ...
      key === todayStr ? "cal-today" : "",
    ].filter(Boolean).join(" ");
    const chips = evs.map((e) => {
      const k = CAL_KIND[e.kind] || { cls: "" };
  ...
  document.getElementById("calLegend").innerHTML = kinds
    .map((k) => CAL_KIND[k]
      ? `<span class="cal-chip ${CAL_KIND[k].cls}">${CAL_KIND[k].label}</span>` : "")
    .join("");
  ...
  note.textContent = until && until < ymd(soon)
    ? `⚠️ 등록된 통화정책 회의 일정이 ${until}까지입니다. meetings.json을 갱신해주세요.`
    : "";
}
